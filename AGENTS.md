...
**No test framework is set up.** Validate changes with `npm run build` (static generation of 199 pages).

## Before making any change

1. **Read this file in full** — it contains the architecture, data flow, file map, and conventions.
2. **Run `npm run build`** after every change. The build statically generates 199 pages ({en,zh} × most routes + 156 card pages + 18 guide pages). If any page fails, the build fails. This is the only validation step (no test suite).
...
- `src/app/(site)/[lang]/guides/[slug]/page.tsx` returns the cartesian product {en,zh} × 9 guide slugs = 18 guide pages.
- Suit landing pages use static route folders (`cards/major/`, `cards/wands/`, …) which take priority over `[id]`.
- `npm run build` generates **199 total pages**.
...
| `/[lang]/guides/[slug]` | SSG (18) | SEO guide hubs for broader search intent |
...
- **Build must pass**: always run `npm run build` after changes. All 199 pages must generate successfully.
